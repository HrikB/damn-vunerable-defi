...
describe('[Challenge] Puppet', function () {
  ...
  it("Execution", async function () {
    const attackPuppetFactory = await ethers.getContractFactory("AttackPuppet");
    const attackPuppet = await attackPuppetFactory.deploy(
      token.address,
      player.address,
      uniswapExchange.address,
      lendingPool.address,
    ...
    const THOUSAND = BigNumber.from("1000000000000000000000");

    await token.connect(player).transfer(attackPuppet.address, THOUSAND);

    await attackPuppet.connect(deployer).pwn();
  });
